...
for i in temp_df.index:
  energy_edge = edge_df.loc[i, 'energy']
  label_string = edge_df.loc[i, 'element'] + ' ' + edge_df.loc[i, 'edge'][:-5]
  ax.axvline(energy_edge, c='k', ls='--', lw=2)
  ax.text(energy_edge, 0.4, label_string, c='k', ha='right', va='top', rotation=90, transform=ax.get_xaxis_transform())
