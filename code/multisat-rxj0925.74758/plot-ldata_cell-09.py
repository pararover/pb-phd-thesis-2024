...
for val in sort_order:
    ...
    energy = data[:,0]
    rate   = data[:,2]
    rate_normalized = (rate - min(rate)) / (max(rate) - min(rate))
    ax.step(energy, rate_normalized, c=plot_colors[sort_order.index(val)],
             where='mid', zorder=sort_order.index(val), label=group_plot[val][1])

ax.set_yscale('log')
ax.set_xlabel(r'Energy (keV)', fontsize=14)
ax.set_ylabel(r'Normalized counts s$^{-1}$ keV$^{-1}$', fontsize=14)
...
op_filename = 'mr-vel-normcounts_all-obs'
